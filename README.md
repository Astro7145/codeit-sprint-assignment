...
## SSR & CSR
<img width="868" height="452" alt="image" src="https://github.com/user-attachments/assets/5201aee6-c7c1-4a3d-b0aa-2bb152d20344" />

기본적으로 Next.js에서 페이지를 SSR로 개발했을 때 얻을 수 있는 이점을 최대한 적극적으로 활용하고자 모든 페이지를 SSR로 개발하고자 구상했습니다. \
그리고 최대한 말단의 컴포넌트에서만 CSR을 활용하여 useQuery를 통해 서버 사이드에서 prefetch된 데이터를 가져오는 방식으로 개발하려고 시도했습니다.

<img width="758" height="580" alt="image" src="https://github.com/user-attachments/assets/d677a512-5c5b-45da-8a46-e3e48c92372e" />

todo 목록 화면에서는 그것이 가능했는데, 각종 입력 상자로부터 데이터를 얻어와 서버에 전송하는 처리가 있는 수정 화면에서는 현실적인 어려움이 있었습니다. \
이를 해결하기 위하여 TodoDetail(수정 화면 컴포넌트)는 SSR로 두고 하위 컴포넌트들(체크박스, 이름, 이미지, 메모 등)에서 전역 상태 관리를 활용하여 데이터를 저장해보고자 시도했습니다.

<img width="587" height="696" alt="image" src="https://github.com/user-attachments/assets/7dc9fef3-7816-4ae0-9434-0a20a3fc7b05" />

하지만 이 또한 각 todo 항목 마다 전역 상태가 업데이트 되도록 관리하는 것이 쉽지 않아서 결국 TodoDetail을 CSR로 개발하여 각 핸들러를 자식 컴포넌트에 전달하는 방법을 택했습니다.

# 사용 설명
접속 링크 - https://codeit-sprint-assignment.vercel.app/
